...
function CreateLaunchpadPitchtoken() {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const getTokensAmount = async () => {
    ...
    if (presale) {
      let x = presale / launchpadDetails.hardCap;
      let y = x * amount;
      setTokensAmount(y);
    }
  };
      ...
      details
        ...
        .then(async (resp) => {
          ...
          setLaunchpadDetails({
            ...
            listingRate: window.web3.utils.fromWei(resp[2]),
  ...
  const userAdress = window.ethereum?.selectedAddress;
  ...
  const getClaimStatus = async () => {
    if (window.ethereum) {
      ...
      const claimtokensStatus = await address.getClaimStatus(userAdress);
  ...
  const userPurchase = async () => {
    if (window.ethereum) {
      ...
      const purchase = await address.userPurchased(
        window.ethereum?.selectedAddress
      );
  ...
  const getClaimableToken = async () => {
    if (window.ethereum) {
      ...
      const tokens = await address.getClaimableToken(
        window.ethereum.selectedAddress
      );
  ...
  const getLaunchpadTokenBalance = async () => {
    ...
    await tokenDetail
      .balanceOfToken(launchpadDetails.presaleAddress)
      .then((balance) => {
  ...
  const handleAddWhiteList = async () => {
    console.log("whitelist", whiteListAddresses, whiteList);
    ...
    if (duplicateElements.length > 0 && errorMessageShown) {
      toast.error("Cannot add Duplicate Addresses", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
                    ...
                    <>
                      <div className="flex-div">
                        <div className="pitchtoken-head">
                          <div className="flex-div">
                            ...
                            <div className="flex-div">
                              <div style={{ display: "flex" }}>
                                <h1>
                                  {launchpadDetails.tokenName + " Presale"}
                                </h1>
                              </div>
                          ...
                          {/* <div className="description text-white mt-3">
                            {dataloaded && data.GET[0]?.metadata?.description
                              ? data.GET[0]?.metadata?.description
                              : ""}
                          </div> */}
                        </div>
                      </div>
                    ...
                    <>
                      ...
                      <div className="text-white">
                        <h6>
                          You will recieve:
                          {tokensAmount
                            ? parseFloat(tokensAmount).toFixed(3) +
                              " " +
                        ...
                        claimTokenStatus ? (
                          ...
                          >
                            {claim ? (
                              ...
                              `Claim token (${
                                claimableTokens
                                  ? parseFloat(claimableTokens).toFixed(4)
                                  : 0
                              }
                                )`
                            )}
...
export default CreateLaunchpadPitchtoken;
